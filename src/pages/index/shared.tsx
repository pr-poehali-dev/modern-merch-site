...
export const NEWS: NewsItem[] = [
  { id: 1, title: 'MGroups запускает новое направление технического мерчандайзинга', desc: 'Мы расширили команду и добавили услуги по логистике и изготовлению торгового оборудования для клиентов.', date: '01.07.2026', author: 'Имя Фамилия', tag: 'новость', img: NEWS_IMG[0] },
  { id: 2, title: 'Как правильно выкладывать товар: 5 проверенных принципов', desc: 'Разбираем базовые правила мерчандайзинга, которые повышают продажи на полке без дополнительных вложений.', date: '28.06.2026', author: 'Тестовый автор', tag: 'статья', img: NEWS_IMG[1] },
  { id: 3, title: 'Зачем нужен тайный покупатель и как читать его отчёты', desc: 'Рассказываем, какие метрики важны в отчётах mystery shopper и как их использовать для роста сервиса.', date: '22.06.2026', author: 'Имя Фамилия', tag: 'полезно знать', img: NEWS_IMG[2] },
  { id: 4, title: 'MGroups расширила географию присутствия до 87 регионов', desc: 'Теперь наши мерчендайзеры работают ещё в 6 новых городах России — подробности в новости.', date: '15.06.2026', author: 'Тестовый автор', tag: 'новость', img: NEWS_IMG[3] },
  { id: 5, title: 'BTL-акции: как выбрать формат под ваш бюджет', desc: 'Сравниваем дегустации, промоакции и праздничные мероприятия — что выбрать для разного бюджета.', date: '10.06.2026', author: 'Имя Фамилия', tag: 'статья', img: NEWS_IMG[0] },
  { id: 6, title: 'Чек-лист аудита торговой точки перед запуском акции', desc: 'Полезный список проверок, которые стоит провести до старта промо-кампании в рознице.', date: '03.06.2026', author: 'Тестовый автор', tag: 'полезно знать', img: NEWS_IMG[1] },
  { id: 7, title: 'Итоги полугодия: рост полевого персонала на 18%', desc: 'Делимся результатами работы команды MGroups за первое полугодие 2026 года.', date: '28.05.2026', author: 'Имя Фамилия', tag: 'новость', img: NEWS_IMG[2] },
  { id: 8, title: 'Планограмма: что это и почему её важно соблюдать', desc: 'Объясняем простыми словами, зачем нужна планограмма и как она влияет на продажи.', date: '20.05.2026', author: 'Тестовый автор', tag: 'статья', img: NEWS_IMG[3] },
  { id: 9, title: '3 ошибки при найме промоперсонала', desc: 'Собрали частые ошибки заказчиков при подборе команды для мероприятий и как их избежать.', date: '14.05.2026', author: 'Имя Фамилия', tag: 'полезно знать', img: NEWS_IMG[0] },
  { id: 10, title: 'Новое мобильное приложение для полевых сотрудников', desc: 'Обновили приложение для мерчендайзеров: удобнее фотоотчёты и быстрее синхронизация данных.', date: '05.05.2026', author: 'Тестовый автор', tag: 'новость', img: NEWS_IMG[1] },
  { id: 11, title: 'Как читать аналитику по представленности товара', desc: 'Разбираем ключевые показатели retail-аудита и как принимать решения на их основе.', date: '27.04.2026', author: 'Имя Фамилия', tag: 'статья', img: NEWS_IMG[2] },
  { id: 12, title: 'Сезонная выкладка: советы к праздничным датам', desc: 'Как заранее подготовить точки продаж к пиковым датам и не потерять продажи.', date: '20.04.2026', author: 'Тестовый автор', tag: 'полезно знать', img: NEWS_IMG[3] },
  {
    id: 13,
    title: 'Что такое шоппер-маркетинг?',
    desc: 'Разбираемся, как шоппер-маркетинг помогает влиять на выбор покупателя прямо у полки магазина.',
    date: '06.07.2026',
    author: 'Тестовый автор',
    tag: 'статья',
    img: 'https://cdn.poehali.dev/projects/f54777b0-87fc-4f92-93d7-a263150798ce/files/6dfe7532-c9dd-47ab-921d-40ad36126835.jpg',
    intro: 'Многие покупатели окончательно решают, что взять, прямо у полки — поэтому одной из главных задач становится не просто завлечь человека в магазин рекламой, а грамотно повлиять на его выбор в тот самый момент. Тут на помощь приходит шоппер‑маркетинг: он изучает, как люди ведут себя в магазине и что подталкивает их к покупке, помогая разобраться, почему клиент отдаёт предпочтение конкретному товару.\n\nНа основе таких исследований формируют практические советы — они позволяют выстраивать взаимодействие с покупателями с учётом психологии и эффективнее склонять их к выбору нужного продукта.',
    content: `
<h2>В чём суть шоппер‑маркетинга: разбираем на пальцах</h2>
<p>Шоппер‑маркетинг делает ставку не на абстрактного потребителя, а на реального человека у полки — на того, кто прямо сейчас решает: брать или не брать. Тут важно уловить разницу: покупатель — это тот, кто достаёт кошелёк и совершает покупку. Его мысли крутятся вокруг практических вещей: как выбрать из десятка похожих вариантов, подойдёт ли товар близким, успею ли всё купить… А потребитель — это чаще тот, кто потом будет этим товаром пользоваться: ребёнок, супруг, питомец. На потребителей работает классическая реклама: она рисует красивые сценарии, играет на эмоциях и имидже. Шоппер‑маркетинг решает другую задачу: убирает препятствия, которые мешают человеку принять решение прямо в торговом зале.</p>
<p>Прежде чем запускать активности в магазине, стоит прояснить несколько ключевых моментов:</p>
<ul>
<li>почему люди выбирают именно эту торговую точку;</li>
<li>с какой целью они пришли — за конкретным товаром или просто «посмотреть»;</li>
<li>что покупают осознанно, а что — спонтанно;</li>
<li>по каким критериям делают выбор;</li>
<li>на какие вывески, ценники и промоакции реально обращают внимание.</li>
</ul>
<h2>На чём строится шоппер‑маркетинг</h2>
<p>В основе подхода — чёткая ориентация на нужды покупателя. Чтобы выстроить грамотную стратегию, нужно глубоко разобраться, как люди воспринимают товары, что их мотивирует и какие внутренние установки влияют на решение о покупке. Для этого проводят детальные исследования: не просто собирают сухие цифры, а стараются понять психологию выбора. Дальше на базе этих данных создают точечные решения — под конкретный продукт и конкретную аудиторию. Важно не останавливаться на внедрении: нужно регулярно оценивать, что сработало, а что нет. И ещё один критический момент — плотная совместная работа производителя и ритейлера: только так можно добиться, чтобы все элементы акции работали в связке и вели к общей цели.</p>
<h2>Чем воздействуют на покупателя: рабочие инструменты</h2>
<div class="tools-grid">
<div class="tools-card"><h3>Привлечь внимание</h3><p>Тут помогают POS‑материалы, продуманное зонирование, понятная навигация и кросс‑мерчандайзинг — когда товары логично дополняют друг друга и подталкивают к дополнительным покупкам.</p></div>
<div class="tools-card"><h3>Сделать удобно</h3><p>Задача — сэкономить время и силы покупателя. В ход идут понятные указатели, перекрёстные предложения, онлайн‑заказ, самовывоз и доставка — всё, что упрощает путь от «хочу» до «купил».</p></div>
<div class="tools-card"><h3>Внедрить инновации</h3><p>Электронные ценники, кассы самообслуживания и «умные» тележки делают процесс покупки быстрее и комфортнее.</p></div>
<div class="tools-card"><h3>Задействовать BTL‑механики</h3><p>Например, промоакции на чеках с розыгрышами — их специально адаптируют под конкретный магазин и аудиторию, чтобы попасть точно в цель.</p></div>
</div>
<div class="cta-callout">
<p>Не хотите погружаться в детали и выстраивать всё самостоятельно? Доверьте задачу профессионалам.</p>
<p>Мы специализируеемся на интенсивных продажах и работает как с крупными международными, так и с российскими компаниями. В нашем арсенале — полный набор решений в сфере мерчандайзинга.</p>
<p>Оставьте заявку через форму на сайте — и мы поможем запустить эффективный проект уже в ближайшее время!</p>
</div>
`,
  },
];
...
export function NewsCard({ item, className }: { item: NewsItem; className?: string }) {
  return (
    <Link to={`/news/${item.id}`} className={`group flex h-full flex-col overflow-hidden rounded-3xl border border-neutral-100 bg-white shadow-sm transition-all hover:shadow-lg ${className || ''}`}>
